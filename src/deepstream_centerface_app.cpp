#include "deepstream_centerface_app.hpp"

namespace CenterFace {
  void
  Nexus::update_fps(gint id) {
  ...
  int
  Nexus::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  Nexus::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  Nexus::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
  ...
  GstPadProbeReturn
  Nexus::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
  ...
  gboolean
  Nexus::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  Nexus::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  Nexus::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  Nexus::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  Nexus::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  Nexus::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  Nexus::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_centerface_detector,
    ...
    // Set all important properties of pgie_centerface_detector
    g_object_set(G_OBJECT(pgie_centerface_detector),
                "config-file-path", PGIE_CENTERFACE_DETECTOR_CONFIG_FILE_PATH, NULL);
    ...
    // Override batch-size of pgie_centerface_detector
    g_object_set(G_OBJECT(pgie_centerface_detector), "batch-size", batch_size, NULL);

    // Check if Engine Exists
    if (boost::filesystem::exists(boost::filesystem::path(
        CenterFace::Nexus::PGIE_CENTERFACE_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_centerface_detector),
                  "model-engine-file", CenterFace::Nexus::PGIE_CENTERFACE_ENGINE_PATH.c_str(), NULL);
    }
    else {
      cout << str(boost::format("CENTERFACE Engine for batch-size: %d and compute-mode: %s not found.")
              % batch_size % COMPUTE_MODE) << endl;
      return EXIT_FAILURE;
    }

    // Set necessary properties of the tracker element
    if (!CenterFace::Nexus::set_tracker_properties(nvtracker)) {
  ...
  void Nexus::setPaths(guint batch_size) {
    // Config Paths
    PGIE_CENTERFACE_DETECTOR_CONFIG_FILE_PATH =
    strdup("models/Centerface/centerface_config.txt");

    TRACKER_CONFIG_FILE =
    strdup("models/Trackers/DCF/ds_tracker_config.txt");

    // Engine Paths
    PGIE_CENTERFACE_ENGINE_PATH =
    str(boost::format("models/Centerface/centerface_544x960.onnx_b%d_gpu0_%s.engine") % batch_size % COMPUTE_MODE);
  }
}

int main(int argc, char *argv[]) {

  CenterFace::Nexus nexus;
  GMainLoop *loop = NULL;
  ...
  ctx = g_option_context_new("Nexus DeepStream App");
  group = g_option_group_new("Nexus", NULL, NULL, NULL, NULL);
  g_option_group_add_entries(group, nexus.entries);
  ...
  pipeline = gst_pipeline_new("centerface-pipeline");
  ...
  gint sources = nexus.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  nexus.setPaths(batch_size);

  // Primary GPU Inference Engine
  pgie_centerface_detector = gst_element_factory_make("nvinfer", "primary-centerface-nvinference-engine");

  if (!pgie_centerface_detector) {
    g_printerr("PGIE CENTERFACE Detector could not be created.\n");
    return -1;
  }
  ...
  int fail_safe = nexus.configure_element_properties(num_sources, streammux, pgie_centerface_detector,
  ...
  /* Set up the pipeline */
    ...
    if (!nexus.display_off) {
      gst_bin_add_many(GST_BIN(pipeline),
                      pgie_centerface_detector, nvtracker, tiler, nvvidconv, nvosd, transform, sink, NULL);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      nexus.tiler_src_pad_buffer_probe, NULL, NULL);
  }
